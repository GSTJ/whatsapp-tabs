...
export default () => {
  ...
  const filterLock = useCallback(
    client => {
      ...
      let owned = false
      users.forEach(user => {
        if (user._id !== profile._id && user.conversations.includes(client._id))
          owned = true
      })
  ...
  const loadMessages = useCallback(
    target => {
      ...
      if (clients.get(target._id) && target.firstLoad)
        return getClientMessages({ variables: { clientID: target._id } })
  ...
  return (
    <Container>
      {!hasConversations ? (
        ...
        (toggle
          ? [...users.values()]
          : [...clients.values()].filter(filterLock)
        )
          .filter(searchFilter)
          .map(target => {
