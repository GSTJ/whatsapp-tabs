...
export default () => {
    ...
    const scrollToBottom = useCallback(ignore => {
        if (!scroll.current) return ignore && setTimeout(() => scrollToBottom(true), 100); // Workaround, may lead to an infinite loop if current is never set
        const { scrollHeight, scrollTop } = scroll.current;
        if (scrollHeight - 1000 < scrollTop || ignore) scroll.current.scrollTop = scrollHeight
    }, [])
