...
import { SEND_USER_MESSAGE, SEND_CLIENT_MESSAGE, UPLOAD_FILE } from "graphql/requests"
...
export default () => {
  ...
  const handleSend = useCallback(vars => {
  ...
  const handleFile = useCallback(response => {
  ...
  const handleUpload = useCallback(async files => {
    Array.from(files).forEach(async File => {
      ...
      if (!/pdf|png|jpeg|jpg|mp3|mp4/.test(type)) return enqueueSnackbar("Tipo de arquivo não suportado.", { variant: "warning" });
