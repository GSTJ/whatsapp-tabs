...
const production: boolean = NODE_ENV === 'production'
const port: number = production ? 80 : 8087
...
export default () => {
  ...
  const customer = useSelector((state: any) => state.customers.get(selected))
  const user = useSelector((state: any) => state.users.get(selected))
  ...
  const handleUpload = useCallback(
    async (files: FileList) => {
      Array.from(files).forEach(async File => {
        ...
        const file: File = { name, type, base64 }
